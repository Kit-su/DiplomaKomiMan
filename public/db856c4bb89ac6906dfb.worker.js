...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/solvers/heuristic-construction/simulatedAnnealing.worker.js");
/******/ })
/************************************************************************/
/******/ ({
...
// orangish
const START_POINT_COLOR = [255, 87, 34];

// blueish
const POINT_COLOR = [41, 121, 255];

// greenish
const BEST_PATH_COLOR = [76, 175, 80];

// orangish
const EVALUATING_PATH_COLOR = [255, 87, 34, 225];

// reddish
const EVALUATING_ERROR_COLOR = [255, 25, 25, 240];
...
const simulatedAnnealing = async points => {
  ...
  const deltaDistance = (aIdx, bIdx) => {
    ...
    let diff =
      Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[bPrev], path[aIdx]) +
      Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[aIdx], path[bNext]) +
      Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[aPrev], path[bIdx]) +
      Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[bIdx], path[aNext]) -
      Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[aPrev], path[aIdx]) -
      Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[aIdx], path[aNext]) -
      Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[bPrev], path[bIdx]) -
      Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[bIdx], path[bNext]);

    if (bPrev === aIdx || bNext === aIdx) {
      diff += 2 * Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[aIdx], path[bIdx]);
    }
    return diff;
  };
  ...
  const initialTemp = 100 * Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[0], path[1]);
  let i = 0;
  ...
  for (
    ...
    if (i % 10000 == 0) {
      self.setEvaluatingPaths(() => ({
        paths: [{ path, color: _constants__WEBPACK_IMPORTED_MODULE_2__["EVALUATING_PATH_COLOR"] }],
    ...
    if (i % 100000 == 0) {
      path.push(sp);
      self.setBestPath(path, Object(_cost__WEBPACK_IMPORTED_MODULE_1__["pathCost"])(path));
      path.pop();
    }
    i++;
  }
  ...
  Object(_cost__WEBPACK_IMPORTED_MODULE_1__["rotateToStartingPoint"])(path, sp);
...
Object(_makeSolver__WEBPACK_IMPORTED_MODULE_0__["default"])(simulatedAnnealing);
...
const makeSolver = solver => {
  ...
  self.setEvaluatingPaths = (getPaths, level = 1) => {
    if (self.solverConfig.detailLevel >= level) {
      ...
      self.postMessage(_store_actions__WEBPACK_IMPORTED_MODULE_0__["setEvaluatingPaths"](paths, cost));
    }
  };
...
const startSolvingAction = (points, delay, evaluatingDetailLevel) => ({
  type: START_SOLVING,
  points,
  delay,
...
const setEvaluatingPaths = (paths, cost) => ({
  type: SET_EVALUATING_PATHS,
  paths,
  cost
});
...
/******/ });
//# sourceMappingURL=db856c4bb89ac6906dfb.worker.js.map